        ...
        if (statement->child_block_.get() == nullptr) {
            if (statement->tokens_.size() == 2 && statement->tokens_[0] == "port") {
                ...
                if (port_number >= 0 && port_number <= 0xffff) {
                    ...
                    std::cerr
                        << "Invalid port number in config file: " << port_number
                        << "\n";
                }
            }
        } else {  // if it has child blocks
            int child_port_number = statement->child_block_->parse_port();
            if (child_port_number == -1) {  // skip invalid port number
                continue;
            } else {
                return child_port_number;
            }
        }
    }
    return -1;
    ...
    while (input->good()) {
        ...
        switch (state) {
            ...
            case TOKEN_STATE_SINGLE_QUOTE:
                // TODO: the end of a quoted token should be followed by
                // whitespace.
...
bool NginxConfigParser::Parse(std::istream* config_file, NginxConfig* config)
{
    ...
    while (true) {
        ...
        // unnessary output for assignment 2
        ...
        if (token_type == TOKEN_TYPE_START) {
            ...
            if (last_token_type != TOKEN_TYPE_STATEMENT_END &&
                // Fix bug for empty blocks
                last_token_type != TOKEN_TYPE_START_BLOCK &&
                // Fix bug for nested blocks
                last_token_type != TOKEN_TYPE_END_BLOCK) {
                // Error.
                break;
            }
            ...
            if (last_token_type != TOKEN_TYPE_STATEMENT_END &&
                last_token_type != TOKEN_TYPE_END_BLOCK &&
                // Fix bug for empty config file
                last_token_type != TOKEN_TYPE_START) {
                // Error.
                break;
            }
            // Fix bug for mismatched brackets in config file
            if (BracketCount != 0) {
                break;
            }
            return true;
